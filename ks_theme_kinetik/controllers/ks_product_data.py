    ...
    def Ks_ProductData(self, **post):
        ...
        if post['snippet_name'] == 'grid':
            ...
            if ks_slide_rec.ks_item_selection_method == 'products':
                ...
                ks_brands_slider = ks_slide_rec.env["product.brand"].search(
                    [("id", "=", ks_slide_rec.ks_product_brand_ids.ids), ("website_published", '=', True)])
        ...
        if ks_brands_slider:
            for brand in ks_brands_slider:
                    values = {
                        ...
                        'brand_img': "/web/image/product.brand/" + str(brand['id']) + "/logo/330x300",
                        'brand_logo': "/web/image/product.brand/" + str(brand['id']) + "/logo/150x150",
                        'brand_id': brand.id,
        ...
        if ks_products:
            for prods in ks_products:
                ...
                values = {
                    ...
                    'brand_name': prods.product_brand_id.name,
...
class Ks_video_snippets(http.Controller):
    ...
    def attachment_add(self, **kwargs):
        attachment = request.env['ir.attachment'].sudo().search([('id', '=', int(kwargs.get('id', 0)))])  #browse(int(kwargs.get('id', 0)))
        return request.make_response(
