            ...
            if search:
                for srch in search.split(" "):
                    domain += [
                        ...
                        ('description_sale', 'ilike', srch), ('product_brand_id.name', 'ilike', srch),
            ...
            if attrib_values:
                ...
                for value in attrib_values:
                    if not attrib:
                        ...
                        domain += [('product_brand_id.id', 'in', ids)]
                    else:
    ...
    def ks_getShopValues(self, page=0, category=None, search='', ppg=False, **post):
        ...
        if 'filter' in post:
            ...
            if filter.__len__() == 2 and filter[0] == "brand":
                # search = filter[1]
                brnd_list = ['0-' + str(
                    request.env["product.brand"].search([("name", "=", filter[1])], limit=1).id)]
